...
describe('Integration | Repository | Certification ', () => {
  ...
  describe('#getCertification', () => {
    ...
    it('should return a not found error when certification does not exist', () => {
      let nonExistantCertificationCourseId = 1;
      while ([certificationCourse.id, certificationCourseWithoutDate.id].includes(nonExistantCertificationCourseId)) {
        ++nonExistantCertificationCourseId;
      }
      // when
      const promise = certificationRepository.getCertification({ id: nonExistantCertificationCourseId });
