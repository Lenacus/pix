...
export default Service.extend({
  ...
  async handleCertificationCandidatesImport(session, certificationCandidatesRaw) {
    if (session.hasMany('certificationCandidates').ids().length > 0) {
      try {
        await this._deleteCertificationCandidatesInSession(session);
      } catch (error) {
        this.displayCertificationCandidateDeletionError();
      }
    }
    try {
      await this._createCertificationCandidateInSession(session, certificationCandidatesRaw);
      this.displaySuccess();
    } catch (error) {
  ...
  async _createCertificationCandidateInSession(session, certificationCandidatesRaw) {
    const certificationCandidatesToSave = [];
    _.each(certificationCandidatesRaw, (certificationCandidateRaw) => {
      const certificationCandidate = this._convertJSONCertificationCandidateToModel(certificationCandidateRaw);
  ...
  async _deleteCertificationCandidatesInSession(session) {
  ...
  _convertJSONCertificationCandidateToModel(certificationCandidateRaw) {
